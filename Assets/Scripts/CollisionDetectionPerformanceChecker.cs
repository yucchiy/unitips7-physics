...
public class CollisionDetectionPerformanceChecker : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Dropdown _collisionDetectionTypeSelector = null;
    ...
    private void InitializeUI()
    {
        if (_spawnButton != null)
        {
            ...
            {
                if (_collisionDetectionTypeSelector == null)
                {
                    return;
                }
                ...
                switch (_collisionDetectionTypeSelector.options[_collisionDetectionTypeSelector.value].text)
                {
                    case "Discrete":
                        SpawnColliders(CollisionDetectionMode.Discrete, count, root);
                        break;
                    case "Continuous":
                        SpawnColliders(CollisionDetectionMode.Continuous, count, root);
                        break;
                    case "Continuous Dynamic":
                        SpawnColliders(CollisionDetectionMode.ContinuousDynamic, count, root);
                        break;
                    case "Continuous Speculative":
                        SpawnColliders(CollisionDetectionMode.ContinuousSpeculative, count, root);
                        break;
                }
            });
        }
    ...
    private void SpawnColliders(CollisionDetectionMode mode, int count, Transform root)
    {
